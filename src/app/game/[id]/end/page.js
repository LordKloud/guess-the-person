      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 24 }}>
          {players.map((p, i) => {
            ...
            return (
              <div key={p.id} onClick={() => setSelected(p.id)} style={{ background: "#1C1A24", borderRadius: 16, padding: "16px 18px", border: isSelected ? `1.5px solid ${COLORS[i % COLORS.length]}` : "1.5px solid #2A2730", cursor: "pointer", display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: COLORS[i % COLORS.length], display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 600, color: "white", flexShrink: 0 }}>{p.name[0].toUpperCase()}</div>
        ...
        <button onClick={confirmWinner} disabled={!selected || confirming} style={{ width: "100%", padding: "15px", background: selected ? "#F5F0E8" : "#1C1A24", color: selected ? "#13111A" : "#333", border: "1px solid #2A2730", borderRadius: 12, fontSize: 13, fontWeight: 500, letterSpacing: "0.08em", fontFamily: "'Outfit', sans-serif", cursor: selected ? "pointer" : "not-allowed" }}>
